"""
GSTR-1 Reconciliation — Complete Script
========================================
Input  1 : DayBook (Tally) — .xlsx
Input  2 : GSTR-1 Govt file — .xls

Output sheets:
  1. B2B Reconciliation       — invoice-wise match vs govt B2B
  2. B2C Summary              — rate-wise match vs govt B2CS
  3. B2C Invoice Detail       — individual B2C invoice list
  4. B2B HSN Detail           — invoice-wise waterfall per HSN (B2B only)
  5. B2C HSN Detail           — invoice-wise waterfall per HSN (B2C only)
  6. All HSN Combined         — invoice-wise waterfall per HSN (all)
  7. HSN B2B Summary          — HSN-wise clubbed totals (B2B) — govt format
  8. HSN B2C Summary          — HSN-wise clubbed totals (B2C) — govt format
  9. Govt HSN Reference       — raw govt HSN sheet
 10. Legend                   — colour codes & logic explanation

HSN Waterfall Logic:
  - Single-HSN invoice  → full Gross / Taxable / CGST / SGST / IGST assigned to that HSN
  - Multi-HSN invoice   → Remaining = Govt_Total − Σ(single allocations); split equally
  - Govt HSN sheet is FINAL cap; Tally Total always = Govt Total per HSN
"""
...
if TALLY_FILE and GOVT_FILE:

    tally_raw = pd.read_excel(
        TALLY_FILE,
        sheet_name=0,
        header=8
    )

    govt_xls = pd.ExcelFile(
        GOVT_FILE,
        engine="xlrd"
    )
...
# Detect B2B / B2C BEFORE converting column to string
# (column is mixed dtype — actual NaN for blank GSTIN, string for filled)
...
tally_raw['Quantity'] = pd.to_numeric(tally_raw['Quantity'], errors='coerce').fillna(0)

# Collect all CGST / SGST / IGST output columns (handles any % suffix like 9%, 5%, etc.)
...
tally_raw['_TotalTax'] = tally_raw['_CGST'] + tally_raw['_SGST'] + tally_raw['_IGST']
tally_raw['_TaxRate']  = tally_raw.apply(
    lambda r: round(r['_TotalTax'] / r['GST SALES'] * 100) if r['GST SALES'] != 0 else 0,
    axis=1
)
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 2 — READ & CLEAN GOVT GSTR-1 FILE
# ══════════════════════════════════════════════════════════════════════════════
...
# Clean Govt B2CS
govt_b2cs.columns = ['Type', 'Place Of Supply', 'Rate', 'Taxable Value',
                     'Cess Amount', 'E-Commerce GSTIN']
govt_b2cs['Rate']          = pd.to_numeric(govt_b2cs['Rate'],          errors='coerce')
govt_b2cs['Taxable Value'] = pd.to_numeric(govt_b2cs['Taxable Value'], errors='coerce')

# Clean Govt HSN
govt_hsn['HSN'] = govt_hsn['HSN'].astype(str).str.strip()
for col in ['Total Value', 'Taxable Value', 'Central Tax Amount',
            'State/UT Tax Amount', 'Integrated Tax Amount', 'Total Quantity']:
    govt_hsn[col] = pd.to_numeric(govt_hsn[col], errors='coerce').fillna(0)

# Aggregate Govt HSN by HSN code (multiple detail rows → one summary per HSN)
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 3 — B2B INVOICE-WISE RECONCILIATION
# ══════════════════════════════════════════════════════════════════════════════
...
for _, tr in tally_b2b.iterrows():
    ...
    if gr.empty:
        b2b_rows.append({
            ...
            'Tax Rate (Tally)':     tr['_TaxRate'],
            'Tax Rate (Govt)':      '',
            'Rate Match':           '❌ NOT IN GOVT',
            'Reverse Charge':       '',
    ...
    overall = ('✅ MATCH' if ok else '❌ MISMATCH') + (' | ⚠️ RC=Y' if rc == 'Y' else '')
    ...
    b2b_rows.append({
        ...
        'Gross Total (Tally)':   tr['Gross Total'],
        'Invoice Value (Govt)':  g['Invoice Value'],
        'Gross Total Match':     gm,
        'Taxable Value (Tally)': tr['GST SALES'],
        'Taxable Value (Govt)':  g['Taxable Value'],
        'Taxable Match':         tm,
        'CGST':                  tr['_CGST'],
        'SGST':                  tr['_SGST'],
        'IGST':                  tr['_IGST'],
...
govt_only = govt_b2b[
    ~govt_b2b['Invoice Number'].isin(tally_b2b['Voucher No.'].astype(str).str.strip())
]

# ══════════════════════════════════════════════════════════════════════════════
# STEP 4 — B2C SUMMARY RECONCILIATION
# ══════════════════════════════════════════════════════════════════════════════
b2c_agg = tally_b2c.groupby('_TaxRate').agg(
    Taxable_Value = ('GST SALES',    'sum'),
    CGST          = ('_CGST',        'sum'),
    SGST          = ('_SGST',        'sum'),
    IGST          = ('_IGST',        'sum'),
    Total_Tax     = ('_TotalTax',    'sum'),
    Count         = ('Voucher No.',  'count'),
...
for rate in all_rates:
    ...
    b2c_rows.append({
        ...
        'Taxable Value (Govt)':   round(g_tax, 2),
        'Taxable Match':          tax_m,
...
for _, r in tally_b2c.iterrows():
    b2c_detail_rows.append({
        ...
        'HSN':                str(r['HSN']).strip(),
        'Gross Total':        r['Gross Total'],
        'GST Sales (Taxable)':r['GST SALES'],
        'CGST':               r['_CGST'],
        'SGST':               r['_SGST'],
        'IGST':               r['_IGST'],
        'Total Tax':          r['_TotalTax'],
        'Tax Rate %':         r['_TaxRate'],
    })
df_b2c_detail = pd.DataFrame(b2c_detail_rows)

# ══════════════════════════════════════════════════════════════════════════════
# STEP 5 — EXPAND TALLY: ONE ROW PER INVOICE PER HSN (deduplicated)
# ══════════════════════════════════════════════════════════════════════════════
exp_rows = []
for _, r in tally_raw.iterrows():
    ...
    for h in hsns:
        exp_rows.append({
            ...
            'Date':        r['Date'],
            'Particulars': clean_party(r['Particulars']),
            'GSTIN':       r['GSTIN/UIN'],
            'HSN':         h,
            'Gross Total': r['Gross Total'],
            'GST SALES':   r['GST SALES'],
            'CGST':        r['_CGST'],
            'SGST':        r['_SGST'],
            'IGST':        r['_IGST'],
            'Quantity':    r['Quantity'],
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 6 — WATERFALL: invoice-level detail rows per HSN
# ══════════════════════════════════════════════════════════════════════════════
def waterfall_detail(hsn_code, df_exp, govt_hsn_agg, filter_b2b=None):
    """
    Returns list of dicts — one row per invoice for this HSN.
    Includes Tally Total / Govt Total / Diff summary rows.
    filter_b2b: True = B2B only | False = B2C only | None = all
    """
    ...
    rows.append({'HSN': hsn_code, 'Description': desc, 'UQC': uqc,
    ...
    rows.append({'HSN': hsn_code, 'Description': desc, 'UQC': uqc,
        'Voucher No.': '── GOVT TOTAL (Final) ──', 'Date': '', 'Party Name': '',
        'GSTIN': '', 'Type': '', 'Multi-HSN': '',
    ...
    dcg = round(tcg - g_cg, 2); dsg = round(tsg - g_sg, 2); dig = round(tig - g_ig, 2)
    ...
    rows[-1]['Row Type'] = 'Spacer'
    return rows


# ══════════════════════════════════════════════════════════════════════════════
# STEP 7 — WATERFALL: per-HSN allocated rows for aggregation (no labels)
# ══════════════════════════════════════════════════════════════════════════════
def waterfall_alloc(hsn_code, df_exp, govt_hsn_agg):
    """
    Returns plain invoice rows with allocated values (for aggregation into summary).
    No Tally Total / Govt Total rows.
    """
    ga = govt_hsn_agg[govt_hsn_agg['HSN'] == hsn_code]
    if ga.empty:
        return []
    ...
    for _, r in multis.iterrows():
        rows.append({'HSN': hsn_code, 'Description': desc, 'UQC': uqc,
            ...
            'IGST':        0.0,
            'Quantity':    0})   # qty not splittable for multi-HSN
    return rows


# Build all allocated rows
all_alloc = []
...
def build_hsn_summary(df_alloc, is_b2b):
    """
    Aggregate per HSN, filter by B2B or B2C.
    Returns DataFrame in Govt HSN format with a TOTAL row.
    """
    ...
    # Grand total row
    num_cols = ['Total Quantity', 'Total Value', 'Taxable Value',
...
# Build detail waterfall dataframes
all_combined, all_b2b_hsn, all_b2c_hsn = [], [], []
for h in govt_hsn_agg['HSN'].tolist():
    all_combined.extend(waterfall_detail(h, df_exp, govt_hsn_agg, None))
    rows_b2b = waterfall_detail(h, df_exp, govt_hsn_agg, True)
    if any(r['Row Type'] == 'Invoice' for r in rows_b2b):
        all_b2b_hsn.extend(rows_b2b)
    rows_b2c = waterfall_detail(h, df_exp, govt_hsn_agg, False)
    if any(r['Row Type'] == 'Invoice' for r in rows_b2c):
        all_b2c_hsn.extend(rows_b2c)
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 8 — EXCEL STYLES & HELPERS
# ══════════════════════════════════════════════════════════════════════════════
FG   = PatternFill("solid", fgColor="C6EFCE")   # green  — match
FR   = PatternFill("solid", fgColor="FFC7CE")   # red    — mismatch
FY   = PatternFill("solid", fgColor="FFEB9C")   # yellow — RC=Y
FT   = PatternFill("solid", fgColor="E2EFDA")   # light green — Tally Total
FV   = PatternFill("solid", fgColor="DDEBF7")   # light blue  — Govt Total
FALT = PatternFill("solid", fgColor="F2F2F2")   # alternating row (grey)
FN   = PatternFill("solid", fgColor="1F4E79")   # navy  — header
FS   = PatternFill("solid", fgColor="2E75B6")   # steel — sub-header
...
def cw(ws, ri, ci, val, fill=None, bold=False, num_fmt=None):
    """Write a cell with border, font, alignment."""
...
ws.cell(1, 1, "GSTR-1 B2B Reconciliation — May 2026").font = Font(bold=True, size=13, color="1F4E79")
ws.cell(2, 1, "🟢 Match  |  🔴 Mismatch / Missing  |  🟡 Reverse Charge = Y").font = Font(size=9, italic=True)
cols = list(df_b2b_result.columns)
hdr(ws, 4, cols)
for ri, (_, row) in enumerate(df_b2b_result.iterrows(), 5):
    status = str(row.get('Overall Status', ''))
    rc = str(row.get('Reverse Charge', ''))

    if '✅ MATCH' in status and rc != 'Y':
        fill = FG
    elif '❌' in status:
        fill = FR
    elif rc == 'Y' or '⚠️' in status:
        fill = FY
    else:
        fill = None
...
ws2.cell(1, 1, "GSTR-1 B2C Summary Reconciliation — May 2026").font = Font(bold=True, size=13, color="1F4E79")
...
ws3.cell(1, 1, "B2C Invoice Detail — Blank GSTIN in Tally").font = Font(bold=True, size=13, color="1F4E79")
...
# ── HSN waterfall detail sheet writer ─────────────────────────────────────────
DISP = ['HSN', 'Description', 'UQC', 'Voucher No.', 'Date', 'Party Name',
...
def write_hsn_detail(wb, df, sheet_name, title):
    ...
    ws.cell(2, 1,
        "🟩 Tally Total  |  🟦 Govt Total (Final)  |  🟢 Match  |  🔴 Diff  "
        "|  Note: Govt HSN is FINAL cap"
    ...
    for _, row in df.iterrows():
        ...
        if rt == 'TallyTotal':
            ...
            try:    dv = float(row.get('Gross Total', 0))
            except: dv = 0
...
write_hsn_detail(wb, df_b2b_hsn,  "B2B HSN Detail",
                 "B2B HSN — Invoice-wise Waterfall vs Govt HSN (May 2026)")
write_hsn_detail(wb, df_b2c_hsn,  "B2C HSN Detail",
                 "B2C HSN — Invoice-wise Waterfall vs Govt HSN (May 2026)")
write_hsn_detail(wb, df_combined, "All HSN Combined",
                 "All HSN (B2B + B2C) — Invoice-wise Waterfall vs Govt HSN (May 2026)")
...
def write_hsn_summary(wb, df, sheet_name, title, subtitle):
    ws = wb.create_sheet(sheet_name)
    ws.cell(1, 1, title).font = Font(bold=True, size=13, color="1F4E79")
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=len(HSN_COLS))
    ws.cell(2, 1, subtitle).font = Font(size=9, italic=True, color="595959")
    ws.merge_cells(start_row=2, start_column=1, end_row=2, end_column=len(HSN_COLS))
    hdr(ws, 3, HSN_COLS)
    ...
    for ri, (_, row) in enumerate(df.iterrows(), 4):
        ...
        for ci, col in enumerate(HSN_COLS, 1):
            ...
            if col in ('HSN', 'UQC', 'Total Quantity'):
                ...
                if col != 'Cess Amount' and not is_total:
                    c.number_format = '#,##0.00'
                elif is_total and col not in ('HSN', 'Description', 'UQC'):
                    c.number_format = '#,##0.00'
            if fill:
                c.fill = fill
...
write_hsn_summary(
    ...
    "HSN-wise clubbed totals for B2B invoices | Govt HSN = Final cap | "
    "Multi-HSN invoice remaining split equally per HSN",
)
write_hsn_summary(
    wb, df_b2c_hsn_sum, "HSN B2C Summary",
    "HSN Summary — B2C Invoices (Tally Waterfall Allocation)",
    "HSN-wise clubbed totals for B2C invoices (blank GSTIN) | Govt HSN = Final cap | "
    "Multi-HSN invoice remaining split equally per HSN",
)

# ── Govt HSN Reference ────────────────────────────────────────────────────────
ws_ref = wb.create_sheet("Govt HSN Reference")
ws_ref.cell(1, 1, "Govt GSTR-1 HSN Sheet — Final Reference").font = Font(bold=True, size=13, color="1F4E79")
...
legend = [
    ...
    ("Step 1 — Single-HSN invoice",
     "Full Gross Total / Taxable Value / CGST / SGST / IGST assigned directly to that HSN"),
    ("Step 2 — Multi-HSN invoice",
     "Remaining = Govt_Total − Σ(single-invoice allocations for this HSN)"),
    ("Step 3 — Split remaining",
     "Remaining divided equally among all multi-HSN invoices that include this HSN"),
    ("Step 4 — Verify",
     "Tally Total = singles + remaining = Govt Total (always matches by design)"),
    ("", ""),
    ("Example — HSN 2903", "Govt Total = 13,570"),
    ("", "CSES/I/074/26-27  (single HSN)  →  3,540   |  running = 3,540"),
    ("", "CSES/I/075/26-27  (single HSN)  →  2,950   |  running = 6,490"),
    ("", "CSES/I/087/26-27  (multi  HSN)  →  13,570 − 6,490 = 7,080"),
    ("", "Tally Total = 3,540 + 2,950 + 7,080 = 13,570  ✅"),
    ("", ""),
    ("HSN B2B / B2C Summary", ""),
    ("HSN B2B Summary sheet",
     "HSN-wise aggregated totals from B2B invoices (GSTIN present in Tally)"),
    ("HSN B2C Summary sheet",
     "HSN-wise aggregated totals from B2C invoices (GSTIN blank in Tally)"),
    ("Format",
...
# ── Save ──────────────────────────────────────────────────────────────────────
OUT_FILE = "GSTR1_Reconciliation_Output.xlsx"
wb.save(OUT_FILE)
...
print(f"\nSummary:")
print(f"  B2B invoices reconciled : {len(df_b2b_result)}")
print(f"  B2C rate rows           : {len(df_b2c_result)}")
print(f"  B2C invoices (detail)   : {len(df_b2c_detail)}")
print(f"  HSN B2B Summary rows    : {len(df_b2b_hsn_sum) - 1}  (+1 total row)")
print(f"  HSN B2C Summary rows    : {len(df_b2c_hsn_sum) - 1}  (+1 total row)")